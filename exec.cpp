...
tacsim::exec::table_t::table_t()
{
	...
	insert(make_pair(tac::ALLOC, alloc));
	insert(make_pair(tac::DEALLOC, dealloc));
...
tacsim::pc_t tacsim::alloc(tacsim::pc_t pc)
{
	using namespace std;
	using namespace COMPILER;
...
tacsim::pc_t tacsim::dealloc(tacsim::pc_t pc)
{
	using namespace std;
	using namespace COMPILER;

	tac* ptr = *pc;
	delete[] getaddr(ptr->y);
	address_table_t& curr = g_stack.back();
	curr.erase(ptr->y);
	return pc + 1;
}
